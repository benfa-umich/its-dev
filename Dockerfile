...
#COPY https://github.com/mrschlei/its-dev/ /var/www/html/
# should we put a Dockerfile in the repo/locally, and expect it to be run from there?
#RUN git clone https://github.com/mrschlei/its-dev/ /var/www/html/

### Start script incorporates config files and sends logs to stdout ###
#COPY its-itdev-start.sh /usr/local/bin
#RUN chmod 755 /usr/local/bin/its-itdev-start.sh
#CMD /usr/local/bin/its-itdev-start.sh
